...
class GetPaginatedCitiesUseCaseAndroidTest {
    ...
    @Test
    fun `invoke returns a Flow of PagingData when favorites are mapped`() = runTest(testDispatcher) {
        // given
    ...
    @Test
    fun `invoke returns a Flow of PagingData when favorites are not mapped`() = runTest(testDispatcher) {
        // given
        val pagingSource = FakeCitiesPagingSource()
